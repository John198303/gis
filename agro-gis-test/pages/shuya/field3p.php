...
require_once '../../funcs/operations_p3.php';

if(!empty($_POST['date_operations'])){
	save_mess_operations_p3();
	header("Location: ../../pages/shuya/field3p.php");
	die;
} 
...
<body>
	...
	<div class="fil clearfix">	
		...
		<div class="field_map_text">
			...
			<div id="field_d"><h2>Поле № 3П (94 Га)</h2>
			...
			<div class="korpus">				
	  			...
	  			<?php
					$sql2 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Сев');";
					$result2 = mysqli_query($db1, $sql2);												
					$followingdata = $result2->fetch_array(MYSQLI_ASSOC);
					
					$sql3 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Культивация');";
					$result3 = mysqli_query($db1, $sql3);
					$followingdata1 = $result3->fetch_array(MYSQLI_ASSOC);
					
					$sql4 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Боронование');";
					$result4 = mysqli_query($db1, $sql4);
					$followingdata2 = $result4->fetch_array(MYSQLI_ASSOC);
					
					$sql5 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Дискование');";
					$result5 = mysqli_query($db1, $sql5);												
					$followingdata3 = $result5->fetch_array(MYSQLI_ASSOC);
					
					$sql6 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Прикатывание');";
					$result6 = mysqli_query($db1, $sql6);												
					$followingdata4 = $result6->fetch_array(MYSQLI_ASSOC);
					
					$sql7 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Сбор камней');";
					$result7 = mysqli_query($db1, $sql7);												
					$followingdata5 = $result7->fetch_array(MYSQLI_ASSOC);
					
					$sql8 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Внесение удобрений');";
					$result8 = mysqli_query($db1, $sql8);												
					$followingdata6 = $result8->fetch_array(MYSQLI_ASSOC);
					
					$sql8 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Опрыскивание');";
					$result8 = mysqli_query($db1, $sql8);												
					$followingdata7 = $result8->fetch_array(MYSQLI_ASSOC);
					
					$sql9 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Подсев');";
						...
						<div class="total">
							...
							<div class="table_div_total">
								<table>
									<tr>
										<td>Сев
											<meter value="<?php echo $followingdata["sum(`done`)"] ?>" max="94" low="0" high="120"></meter>
										</td>
										<td>
										<?php
												$sql2 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Сев');";
												...
												echo "<span class='total_span'>" . $followingdata["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata["sum(`done`)"]/94*100, 1). "%"; ?>  
										
										</td>
									</tr>
									<tr>
										<td>Культивация
											<meter value="<?php echo $followingdata1["sum(`done`)"] ?>" max="94" low="0" high="120"></meter>
										</td>
										<td>
											<?php
												$sql3 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Культивация');";
												...
												echo "<span class='total_span'>" . $followingdata1["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata1["sum(`done`)"]/94*100, 1). "%"; ?> 	
										
										</td>
									</tr>
									<tr>
										<td>Боронование
											<meter value="<?php echo $followingdata2["sum(`done`)"] ?>" max="94" low="0" high="120"></meter>
										</td>
										<td>
											<?php
												$sql3 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Боронование');";
												$result3 = mysqli_query($db1, $sql3);
												$followingdata1 = $result3->fetch_array(MYSQLI_ASSOC);
									...
									<tr>
										<td>Дискование
											<meter value="<?php echo $followingdata3["sum(`done`)"] ?>" max="94" low="0" high="120"></meter>
										</td>
										<td>
											<?php
												$sql4 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Дискование');";
												$result4 = mysqli_query($db1, $sql4);
												
												$followingdata2 = $result4->fetch_array(MYSQLI_ASSOC);
												
												echo "<span class='total_span'>" . $followingdata2["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata2["sum(`done`)"]/94*100, 1). "%"; ?>  		
										</td>
									</tr>
									<tr>
										<td>Прикатывание
											<meter value="<?php echo $followingdata4["sum(`done`)"] ?>" max="94" low="0" high="120"></meter>
										</td>
										<td>
											<?php
												$sql5 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Прикатывание');";
												$result5 = mysqli_query($db1, $sql5);
												
												$followingdata3 = $result5->fetch_array(MYSQLI_ASSOC);
												
												echo "<span class='total_span'>" . $followingdata3["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata3["sum(`done`)"]/94*100, 1). "%"; ?> 
										</td>
									</tr>
									<tr>
										<td>Сбор камней
											<meter value="<?php echo $followingdata5["sum(`done`)"] ?>" max="94" low="0" high="120"></meter>
										</td>
										<td>
											<?php
												$sql6 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Сбор камней');";
												$result6 = mysqli_query($db1, $sql6);
												
												$followingdata4 = $result6->fetch_array(MYSQLI_ASSOC);
												
												echo "<span class='total_span'>" . $followingdata4["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata4["sum(`done`)"]/94*100, 1). "%"; ?> 
										</td>
									</tr>
									<tr>
										<td>Внесение удобрений
											<meter value="<?php echo $followingdata6["sum(`done`)"] ?>" max="94" low="0" ></meter>
										</td>
										<td>
											<?php
												$sql7 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Внесение удобрений');";
												$result7 = mysqli_query($db1, $sql7);
												
												$followingdata5 = $result7->fetch_array(MYSQLI_ASSOC);
												
												echo "<span class='total_span'>" . $followingdata5["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata5["sum(`done`)"]/94*100, 1). "%"; ?>  
										</td>
									</tr>
									<tr>
										<td>Опрыскивание
											<meter value="<?php echo $followingdata7["sum(`done`)"] ?>" max="94" low="0" ></meter>
										</td>
										<td>
											<?php
												$sql8 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Опрыскивание');";
												...
												echo "<span class='total_span'>" . $followingdata6["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata6["sum(`done`)"]/94*100, 1). "%"; ?> 
												
										</td>
									</tr>
									<tr>
										<td>Подсев
											<meter value="<?php echo $followingdata8["sum(`done`)"] ?>" max="94" low="0" ></meter>
										</td>
										<td>
											<?php
												$sql9 =  "SELECT sum(`done`) FROM `operations-p3` WHERE `operations` in ('Подсев');";
												...
												echo "<span class='total_span'>" . $followingdata7["sum(`done`)"].'&nbsp Га </span> <br> <span class=\'total_span_two\'>выполнено &nbsp'. round($percent = $followingdata7["sum(`done`)"]/94*100, 1). "%"; 
										...
										<tbody>
											...
											<tr>
												...
												<td colspan="2"> Израсходовано %
													<br><meter value="59" max="94" low="0"></meter>
												</td>
									...
									<table class="seed_table">
										...
										<tbody>
											...
											<tr>
												...
												<td colspan="2"> Израсходовано %
													<br><meter value="115" max="94" low="0"></meter>
												</td>
						...
						<div class="table_div">  
							...
							<div class="table-responsive">  
								<table id="editable_table" class="operation_table">
									 ...
									 <tbody>
									 <?php
										$sql =  "SELECT  id, date_operations, operations, main, trailer, driver, done FROM `operations-p3` ORDER BY date_operations DESC LIMIT 20";
						...
						<div class="popup-body">  <span class="popup-exit"></span>
							<div class="popup-content">
								...
								<div class="flex_container">
									<form method = "post" action="../../pages/shuya/field3p.php" class="form_staff">
