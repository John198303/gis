...
require_once '../../funcs/operations_p2.php';

if(!empty($_POST['date_operations'])){
	save_mess_operations_p2();
	header("Location: ../../pages/shuya/field2p.php");
	die;
} 
...
<body>
	...
	<div class="fil clearfix">	
		...
		<div class="field_map_text">
			...
			<div id="field_d"><h2>Поле № 2П (90 Га)</h2>
			...
			<div class="korpus">				
	  			...
	  			<?php
					$sql2 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Сев');";
					$result2 = mysqli_query($db1, $sql2);												
					$followingdata = $result2->fetch_array(MYSQLI_ASSOC);
					
					$sql3 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Культивация');";
					$result3 = mysqli_query($db1, $sql3);
					$followingdata1 = $result3->fetch_array(MYSQLI_ASSOC);
					
					$sql4 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Боронование');";
					$result4 = mysqli_query($db1, $sql4);
					$followingdata2 = $result4->fetch_array(MYSQLI_ASSOC);
					
					$sql5 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Дискование');";
					$result5 = mysqli_query($db1, $sql5);												
					$followingdata3 = $result5->fetch_array(MYSQLI_ASSOC);
					
					$sql6 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Прикатывание');";
					$result6 = mysqli_query($db1, $sql6);												
					$followingdata4 = $result6->fetch_array(MYSQLI_ASSOC);
					
					$sql7 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Сбор камней');";
					$result7 = mysqli_query($db1, $sql7);												
					$followingdata5 = $result7->fetch_array(MYSQLI_ASSOC);
					
					$sql8 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Внесение удобрений');";
					$result8 = mysqli_query($db1, $sql8);												
					$followingdata6 = $result8->fetch_array(MYSQLI_ASSOC);
					
					$sql8 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Опрыскивание');";
					$result8 = mysqli_query($db1, $sql8);												
					$followingdata7 = $result8->fetch_array(MYSQLI_ASSOC);
					
					$sql9 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Подсев');";
							...
							<div class="table_div_total">
								<table>
									...
									<tr>
										...
										<td>
											<?php
												$sql3 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Боронование');";
												$result3 = mysqli_query($db1, $sql3);
												$followingdata1 = $result3->fetch_array(MYSQLI_ASSOC);
									...
									<tr>
										...
										<td>
											<?php
												$sql4 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Дискование');";
												$result4 = mysqli_query($db1, $sql4);
									...
									<tr>
										...
										<td>
											<?php
												$sql5 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Прикатывание');";
												$result5 = mysqli_query($db1, $sql5);
									...
									<tr>
										...
										<td>
											<?php
												$sql6 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Сбор камней');";
												$result6 = mysqli_query($db1, $sql6);
									...
									<tr>
										<td>Внесение удобрений
											<meter value="<?php echo $followingdata6["sum(`done`)"] ?>" max="90" low="0" ></meter>
										</td>
										<td>
											<?php
												$sql7 =  "SELECT sum(`done`) FROM `operations-p2` WHERE `operations` in ('Внесение удобрений');";
												$result7 = mysqli_query($db1, $sql7);
						...
						<div class="table_div">  
							...
							<div class="table-responsive">  
								<table id="editable_table" class="operation_table">
									 ...
									 <tbody>
									 <?php
										$sql =  "SELECT  id, date_operations, operations, main, trailer, driver, done FROM `operations-p2` ORDER BY date_operations DESC LIMIT 20";
						...
						<div class="popup-body">  <span class="popup-exit"></span>
							<div class="popup-content">
								...
								<div class="flex_container">
									<form method = "post" action="../../pages/shuya/field2p.php" class="form_staff">
