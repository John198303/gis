...
$result = mysqli_query($db1," SELECT id, date_operations,operations,main,trailer,driver,done FROM `operations-x3a`
                         WHERE id='$id'   
");
...
if(isset($_POST['save']))
{
   ...
   $result3 = mysqli_query($db1, " UPDATE `operations-x3a` SET date_operations='$date_operations', operations='$operations', main='$main', trailer='$trailer', driver='$driver', done='$done' WHERE id='$id' ");
    
	if ($result3 == TRUE){
		echo "Данные успешно изменены";
		header("Location: ../../../../agro-gis-test/pages/shuya/field3ax.php");
		}else{
			header("Location: .");
		};
...
?>
<div id="container">
<form id="form" class="topBefore" method="post" action="edit_operations-x3a.php?id=<?php echo $id; ?>">
